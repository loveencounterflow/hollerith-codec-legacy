...
(function() {
  //###########################################################################################################
  var CND, CODEC, alert, badge, blob, buffer_as_text, debug, decode, echo, encode, help, info, jrx, log, rpr, test, urge, warn, whisper, ƒ;
  ...
  this["test: flat file DB storage (2)"] = function(T) {
    ...
    probes_and_matchers = [[['foo', -2e308], '⡔⡦⡯⡯⠀⡊![ \'foo\', -Infinity ]'], [['foo', -1000000000000], '⡔⡦⡯⡯⠀⡋⢽⢒⣥⡫⡝⣿⣿⣿![ \'foo\', -1000000000000 ]'], [['foo', -3], '⡔⡦⡯⡯⠀⡋⢿⣷⣿⣿⣿⣿⣿⣿![ \'foo\', -3 ]'], [['foo', -2], '⡔⡦⡯⡯⠀⡋⢿⣿⣿⣿⣿⣿⣿⣿![ \'foo\', -2 ]'], [['foo', -1], '⡔⡦⡯⡯⠀⡋⣀⠏⣿⣿⣿⣿⣿⣿![ \'foo\', -1 ]'], [['foo', 1], '⡔⡦⡯⡯⠀⡌⠿⣰⠀⠀⠀⠀⠀⠀![ \'foo\', 1 ]'], [['foo', 2], '⡔⡦⡯⡯⠀⡌⡀⠀⠀⠀⠀⠀⠀⠀![ \'foo\', 2 ]'], [['foo', 3], '⡔⡦⡯⡯⠀⡌⡀⠈⠀⠀⠀⠀⠀⠀![ \'foo\', 3 ]'], [['foo', 1000000000000], '⡔⡦⡯⡯⠀⡌⡂⡭⠚⢔⢢⠀⠀⠀![ \'foo\', 1000000000000 ]'], [['foo', 2e308], '⡔⡦⡯⡯⠀⡍![ \'foo\', Infinity ]'], [['bar', 'blah'], '⡔⡢⡡⡲⠀⡔⡢⡬⡡⡨⠀![ \'bar\', \'blah\' ]'], [['bar', 'gnu'], '⡔⡢⡡⡲⠀⡔⡧⡮⡵⠀![ \'bar\', \'gnu\' ]'], [['a'], '⡔⡡⠀![ \'a\' ]'], [['b'], '⡔⡢⠀![ \'b\' ]'], [['c'], '⡔⡣⠀![ \'c\' ]'], [['A'], '⡔⡁⠀![ \'A\' ]'], [['箲'], '⡔⣧⢮⢲⠀![ \'箲\' ]'], [['筅'], '⡔⣧⢭⢅⠀![ \'筅\' ]'], [['𥬗'], '⡔⣰⢥⢬⢗⠀![ \'𥬗\' ]'], [['B'], '⡔⡂⠀![ \'B\' ]'], [['C'], '⡔⡃⠀![ \'C\' ]'], [['0'], '⡔⠰⠀![ \'0\' ]'], [['1'], '⡔⠱⠀![ \'1\' ]'], [['2'], '⡔⠲⠀![ \'2\' ]'], [['Number', 2.220446049250313e-16, 'EPSILON'], '⡔⡎⡵⡭⡢⡥⡲⠀⡌⠼⢰⠀⠀⠀⠀⠀⠀⡔⡅⡐⡓⡉⡌⡏⡎⠀![ \'Number\', 2.220446049250313e-16, \'EPSILON\' ]'], [['Number', 9007199254740991, 'MAX_SAFE_INTEGER'], '⡔⡎⡵⡭⡢⡥⡲⠀⡌⡃⠿⣿⣿⣿⣿⣿⣿⡔⡍⡁⡘⡟⡓⡁⡆⡅⡟⡉⡎⡔⡅⡇⡅⡒⠀![ \'Number\', 9007199254740991, \'MAX_SAFE_INTEGER\' ]'], [['Number', 1.7976931348623157e+308, 'MAX_VALUE'], '⡔⡎⡵⡭⡢⡥⡲⠀⡌⡿⣯⣿⣿⣿⣿⣿⣿⡔⡍⡁⡘⡟⡖⡁⡌⡕⡅⠀![ \'Number\', 1.7976931348623157e+308, \'MAX_VALUE\' ]'], [['Number', 0, 'ZERO'], '⡔⡎⡵⡭⡢⡥⡲⠀⡌⠀⠀⠀⠀⠀⠀⠀⠀⡔⡚⡅⡒⡏⠀![ \'Number\', 0, \'ZERO\' ]'], [['Number', -9007199254740991, 'MIN_SAFE_INTEGER'], '⡔⡎⡵⡭⡢⡥⡲⠀⡋⢼⣀⠀⠀⠀⠀⠀⠀⡔⡍⡉⡎⡟⡓⡁⡆⡅⡟⡉⡎⡔⡅⡇⡅⡒⠀![ \'Number\', -9007199254740991, \'MIN_SAFE_INTEGER\' ]'], [['Number', 5e-324, 'MIN_VALUE'], '⡔⡎⡵⡭⡢⡥⡲⠀⡌⠀⠀⠀⠀⠀⠀⠀⠁⡔⡍⡉⡎⡟⡖⡁⡌⡕⡅⠀![ \'Number\', 5e-324, \'MIN_VALUE\' ]']];
  ...
  this["test: ordering where negatives precede void"] = function(T) {
    var buffer_as_text, encode, j, jrx, len, probe, probes;
    probes = [[10], [10, 0], [10, -1], [10, -2], [10, 1], [10, 2], [10, 4], [10, 4, 0], [10, 4, -1], [10, 4, -2], [10, 4, 1], [10, 4, 2], [10, 0, 3], [10, -1, 3], [10, -2, 3], [10, 1, 3], [10, 2, 3], [], [0], [-1], [1], [1, 0], [1, -1], [1, 1]];
    buffer_as_text = function(buffer) {
    ...
    probes = (function() {
      ...
      for (j = 0, len = probes.length; j < len; j++) {
        probe = probes[j];
        results.push([jrx(probe), encode(probe)]);
      }
      return results;
    })();
    probes.sort(function(a, b) {
