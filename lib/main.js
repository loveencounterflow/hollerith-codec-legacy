...
(function() {
  ...
  tm_pnumber = this['typemarkers']['pnumber'] = 'L'.codePointAt(0); // 0x4c

  tm_pinfinity = this['typemarkers']['pinfinity'] = 'M'.codePointAt(0); // 0x4d
  ...
  this.read_nnumber = function(buffer, idx) {
    ...
    copy = this._invert_buffer(new Buffer(buffer.slice(idx, idx + bytecount_number)), 0);
  ...
  this.as_sortline = function(key, settings) {
    ...
    buffer_txt = ((function() {
      var j, ref4, results;
      results = [];
      for (idx = j = 0, ref4 = buffer.length; (0 <= ref4 ? j < ref4 : j > ref4); idx = 0 <= ref4 ? ++j : --j) {
