(function() {
  ...
  write_private = function(idx, value) {
    ...
    while (!(rbuffer.length >= idx + bytecount_typemarker)) {
      grow_rbuffer();
    }
  ...
  read_private = function(buffer, idx) {
  ...
  _decode = function(buffer, idx, single) {
    ...
    while (true) {
      ...
      switch (type = buffer[idx]) {
        ...
        case tm_date:
          ...
          break;
        case tm_private:
          ref7 = read_private(buffer, idx), idx = ref7[0], value = ref7[1];
          break;
        default:
